...
public class SnakeView extends View {
	...
	public SnakeView(Context context,int width,int height,TextView scoreboard,int difficulty,
			int level) {
...
//			            		   | Intent.FLAG_ACTIVITY_NO_HISTORY);
	...
	private void Load_obstructions() {
		...
		for(int i  =0 ; i < obstructions.size();i++) {
			if(obstructions.get(i).getShape().equals("Rectangle")) {
				obstructions.get(i).setTop((int)( (obstructions.get(i).getTop()/100.0) * screen_height));
				obstructions.get(i).setBottom( screen_height - (int)((obstructions.get(i).getBottom()/100.0) * screen_height));
				obstructions.get(i).setLeft((int)((obstructions.get(i).getLeft()/100.0) * screen_width));
				obstructions.get(i).setRight( screen_width - (int)((obstructions.get(i).getRight()/100.0) * screen_width));
		
			}
			else if(obstructions.get(i).getShape().equals("Circle") ){
				obstructions.get(i).setRadius((int)((obstructions.get(i).getRadius()/100.0) * screen_width));
				obstructions.get(i).setCircle_x((int)((obstructions.get(i).getCircle_x()/100.0) * screen_width));
				obstructions.get(i).setCircle_y((int)((obstructions.get(i).getCircle_y()/100.0) * screen_height));
	...
	private boolean location_ok() {
		...
		for( int i = 0 ; i < obstructions.size() ; i++) {
			if(obstructions.get(i).getShape().equals("Rectangle")) {
				...
				int radius = obstructions.get(i).getRadius() + food_radius;
	...
	private void check_game_over() {
		...
		for(int i = 0 ;  i < obstructions.size(); i++) {
			...
			if(obstructions.get(i).getShape().equals("Rectangle")) {
				...
				int radius = obstructions.get(i).getRadius();
				int circle_x = obstructions.get(i).getCircle_x();
				int circle_y = obstructions.get(i).getCircle_y();
				top -= circle_y;
				left -= circle_x;
				bot -= circle_y;
				right -=circle_x;
