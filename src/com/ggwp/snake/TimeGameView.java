...
public class TimeGameView extends View {
	...
	TextView TimerTV;
	TextView BonusTimeTV;
	int timeLeft;
	
	public TimeGameView(Context context,int width,int height,TextView scoreboard,TextView timerTV,
						TextView bonusTimeTV,int difficulty, int level) {
		...
		*/
		...
		while(!checkFoodLocation()) {
			food_y = randInt.nextInt(  (screen_height-food_radius) - (margin+food_radius)  - 200 ) + (margin+food_radius);
	    	food_x = randInt.nextInt(  (screen_width-food_radius) - (margin+food_radius)  )+(margin+food_radius);
		}
		
		TimerTV.setText("00:"+timeLeft);
	...
	public void Pause() {
		...
		if(pause_game) {
			...
			pause_game = false;
			((TimeGameActivity)mcontext).ResumeTimer();
		}
		else {
			((TimeGameActivity)mcontext).PauseTimer();
			pause_game = true;
		}
	}
	...
	public void gameOver() {
    	...
    	if(DB.getHighScore(difficulty, level,TIME_MODE) < score) {
    		DB.updateHighScore(difficulty, level, score,TIME_MODE);
    		...
    		builder.setPositiveButton("Try Again", new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface dialog, int which) {
					...
					activity.finish();
		               ...
		               Intent intent = new Intent(mcontext,TimeGameActivity.class);
		               if(difficulty == 0) {
	 ...
	 * or obstructions
	 */
	private boolean checkFoodLocation() {
	...
	*/
	 ...
	 * 
	 */
	private boolean snakeCollision(int index) {
	...
	private boolean checkFoodEaten() {
		...
		if( top*top + left*left <= food_radius*food_radius ||//top left
			...
			if(difficulty == 0) {
				timeLeft += 5;
				BonusTimeTV.setText(" +"+5);
			}
			
			else if(difficulty == 1) {
				timeLeft += 3;
				BonusTimeTV.setText(" +"+3);
			}
			else if(difficulty == 2) {
				timeLeft += 2;
				BonusTimeTV.setText(" +"+2);
			}
			...
			//create fading bonus time effect
			ObjectAnimator anim = ObjectAnimator.ofFloat(BonusTimeTV, "alpha", 1f, 0f);
	...
	/*
	 * sets the time in the textview timer
	 */
	public void setTime() {
